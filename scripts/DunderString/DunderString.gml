function DunderString() : DunderBaseStruct() constructor {
	...
	static length = function() { return string_length(value); }
	...
	static __mul__ = function(_other) {
		if (not is_numeric(_other)) {
			throw __dunder__.init(DunderExceptionBadType, "__mul__ expected numerical type");	
		}
		return string_repeat(value, _other);
	}
	...
	static __getitem__ = function(_index) {
		...
		if (_index <= -_len or _index >= _len) {
			throw __dunder__init(DunderExceptionIndexError, "Index "+string(_index)+" out of range (0-"+string(_len-1)+")");
		}
