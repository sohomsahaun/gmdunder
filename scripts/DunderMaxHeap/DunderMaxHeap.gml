function DunderMaxHeap() : DunderBaseStruct() constructor { REGISTER_SUBTYPE(DunderMaxHeap);
	...
	static insert = function(_value, _priority) {
		...
		__shift_up(array_length(__values) - 1);
	}
	
	static pop_max = function() {
		var _result = __values[0];
		
		var _last = array_length(__values) -1
		__values[0] = __values[_last];
	...
	static __shift_down = function(_idx) {
		...
		if (_right <= _size and __values[_right][1] > __values[_max_idx][1]) {
			_max_idx = _right;	
		}
